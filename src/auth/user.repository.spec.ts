...
describe('UserRepository', () => {
  ...
  describe('signUp', () => {
    ...
    it('throws a conflict exception as username already exists', () => {
      save.mockRejectedValue({ code: PostgresError.UNIQUE_VIOLATION });
      return expect(userRepository.signUp(mockCredentialsDto)).rejects.toThrow(
        ConflictException,
      );
    });

    it('throws a internal server error exception as smth wrong', () => {
  ...
  describe('hashPassword', () => {
    it('calls brypt.hash to generate a hash', async () => {
      bcrpytjs.hash = jest.fn().mockResolvedValue('testHash');
      ...
      expect(bcrpytjs.hash).toHaveBeenCalledWith('testPassword', 'testSalt');
